...
class Multi_DiceLoss(nn.Module):
    def __init__(self, class_num=3, smooth=0.00001):
...
class DiceLoss_s(nn.Module):
    ...
    def forward(self, input, target):
        ...
        intersect = (input * target).sum()
        union = torch.sum(input) + torch.sum(target)
        # print("s_intersect", intersect)
        # print("s_torch.sum(input), torch.sum(target)", torch.sum(input), torch.sum(target))
        # print("s_sigmoid_round", torch.round(input).sum())
...
class DiceLoss_s_t_20(nn.Module):
    def __init__(self, smooth=0.01):
        super(DiceLoss_s_t_20, self).__init__()
        self.smooth = smooth
    ...
    def forward(self, input, target):
        ...
        intersect = (input * target).sum()
        union = torch.sum(input) + torch.sum(target)
        # print("s_t_20_intersect", intersect)
        # print("s_t_20_torch.sum(input), torch.sum(target)", torch.sum(input), torch.sum(target))
        # print("s_t_20_sigmoid_round", torch.round(input).sum())
