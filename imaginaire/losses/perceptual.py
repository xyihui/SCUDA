...
class PerceptualLoss(nn.Module):
    ...
    Args:
       network (str) : The name of the loss networkss: 'vgg16' | 'vgg19'.
...
class _PerceptualNetwork(nn.Module):
    r"""The networkss that extracts features to compute the perceptual loss.

    Args:
        network (nn.Sequential) : The networkss that extracts features.
    ...
    def __init__(self, network, layer_name_mapping, layers):
        ...
        assert isinstance(network, nn.Sequential), \
            'The networkss needs to be of type "nn.Sequential".'
