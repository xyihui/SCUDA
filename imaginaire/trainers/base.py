...
class BaseTrainer(object):
    ...
    Args:
        ...
        net_G (obj): Generator networkss.
        net_D (obj): Discriminator networkss.
        opt_G (obj): Optimizer for the generator networkss.
        opt_D (obj): Optimizer for the discriminator networkss.
    ...
    def save_checkpoint(self, current_epoch, current_iteration):
        r"""Save networkss weights, optimizer parameters, scheduler parameters
        to a checkpoint.
        """
    ...
    def load_checkpoint(self, cfg, checkpoint_path, resume=None, load_sch=True):
        r"""Load networkss weights, optimizer parameters, scheduler parameters
        from a checkpoint.
        ...
        Args:
            ...
            resume (bool or None): If not ``None``, will determine whether or
                not to load optimizers in addition to networkss weights.
        """
        if os.path.exists(checkpoint_path):
            # If checkpoint_path exists, we will load its weights to
            # initialize our networkss.
            if resume is None:
                resume = False
        ...
        if resume:
            ...
            if not self.is_inference:
                ...
                if 'opt_G' in checkpoint:
                    ...
                    print('Load networkss weights only.')
        else:
            try:
                self.net_G.load_state_dict(checkpoint['net_G'], strict=self.cfg.trainer.strict_resume)
