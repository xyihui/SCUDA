...
class COCOFUNITTranslator(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_res_blocks=2,
                 num_mlp_blocks=3,
                 num_downsamples_style=2,
                 num_downsamples_content=2,
                 num_image_channels=3,
                 weight_norm_type='',
                 **kwargs):
        super().__init__()
        self.conv_s = nn.Sequential(nn.AdaptiveAvgPool2d(1),
                                    nn.Conv2d(num_filters*(2**num_downsamples_style), num_filters, 1, 1, 0),
                                    nn.BatchNorm2d(num_filters),
                                    )
        self.att_style = SelfAttention(2, 1024, 2, 0)
        self.att_content = SelfAttention(2, 1024, 2, 0)
        self.conv = nn.Sequential(nn.Conv2d(256 + 3, 256, 3, 1, 1),
                                  nn.BatchNorm2d(256),
                                  nn.ReLU(inplace=True))
        self.conv_style = nn.Sequential(nn.Conv2d(256 + 3, 256, 3, 1, 1),
                                        nn.BatchNorm2d(256),
                                        nn.ReLU(inplace=True))
        ...
        self.mlp_style = MLP(style_dims*8 + usb_dims,
                             style_dims*8,
                             num_filters_mlp,
    ...
    def decode(self, content, style, real_kv=None, real_kh=None, real_ks=None, real_B_kv=None, real_B_kh=None, real_B_ks=None):
        ...
        # style = self.conv_s(style)
        atte_style = self.att_style(style.reshape(style.size(0), style.size(1), -1)).reshape(2, -1)
        atte_content = self.att_content(style.reshape(style.size(0), style.size(1), -1)).reshape(2, -1)
        ...
        style_in = self.mlp_style(torch.cat([atte_style, usb], 1))  # 风格向量
        coco_style = style_in * atte_content
