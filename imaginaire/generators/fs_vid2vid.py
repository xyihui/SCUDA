...
class Generator(nn.Module):
    ...
    def init_temporal_network(self, cfg_init=None):
        r"""When starting training multiple frames, initialize the flow networkss.
        ...
        if self.warp_ref:
            if self.sep_prev_flownet:
                ...
                print('Initialized temporal flow networkss with the reference '
                      'one.')
            if self.sep_prev_embedding:
                self.init_network_weights(self.ref_image_embedding,
                                          self.prev_image_embedding)
                print('Initialized temporal embedding networkss with the '
                      'reference one.')
    ...
    def load_pretrained_network(self, pretrained_dict, prefix='module.'):
        r"""Load the pretrained networkss into self networkss.

        Args:
            pretrained_dict (dict): Pretrained networkss weights.
            prefix (str): Prefix to the networkss weights name.
        """
        # print(pretrained_dict.keys())
        model_dict = self.state_dict()
        print('Pretrained networkss has fewer layers; The following are '
              'not initialized:')
    ...
    def reset(self):
        r"""Reset the networkss at the beginning of a sequence."""
...
class WeightGenerator(nn.Module):
    ...
    def __init__(self, gen_cfg, data_cfg):
        ...
        if getattr(embed_cfg, 'num_filters', 32) != num_filters:
            raise ValueError('Embedding networkss must have the same number of '
        ...
        # Conv kernel size in embedding networkss.
        ...
        # Use adaptive weight generation for SPADE.
        self.use_hyper_spade = hyper_cfg.is_hyper_spade
        # Use adaptive for the label embedding networkss.
        ...
        # Reference image encoding networkss.
    ...
    def forward(self, ref_image, ref_label, label, is_first_frame):
        r"""Generate networkss weights based on the reference images.
        ...
        # If the reference image has changed, recompute the networkss weights.
        if self.training or is_first_frame or k > 1:
            ...
            # print('Reusing networkss weights.')
            embedding_weights, conv_weights, norm_weights \
                = self.embedding_weights, self.conv_weights, self.norm_weights
    ...
    def get_norm_weights(self, x, i):
        ...
        # Weights for the label embedding networkss.
...
class AttentionModule(nn.Module):
    ...
    def attention_encode(self, img, net_name):
        ...
        Args:
            ...
            net_name (str): Name for attention networkss.
        Returns:
...
class LabelEmbedder(nn.Module):
    ...
    Args:
        emb_cfg (obj): Embed networkss configuration.
    ...
    def forward(self, input, weights=None):
        r"""Embedding networkss forward.

        Args:
            input (NxCxHxW tensor): Network input.
            weights (list of tensors): Conv weights if using hyper networkss.
        Returns:
        ...
        for i in range(self.num_downsamples):
            ...
            # For hyper networkss, the hyper layers are at the last few layers
            # of decoder (if the networkss has a decoder). Otherwise, the hyper
            # layers will be at the first few layers of the networkss.
        ...
        # If the networkss has a decoder, will use outputs from the decoder
