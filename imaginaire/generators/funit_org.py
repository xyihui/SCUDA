...
from models.imaginaire.layers import \
...
class Generator(nn.Module):
    ...
    def __init__(self, gen_cfg=None, data_cfg=None):
        ...
        self.generator = FUNITTranslator()
...
class StyleEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        model1 = []
        model1 += [Conv2dBlock(image_channels, num_filters, 7, 1, 3,
                               **conv_params)]
        for i in range(num_downsamples):
            model1 += [Conv2dBlock(num_filters, 2 * num_filters, 4, 2, 1,
                                   **conv_params)]
            num_filters *= 2
        model1 += [Conv2dBlock(num_filters, num_filters, 4, 2, 1, **conv_params)]
        for _ in range(2):
            model1 += [Res2dBlock(num_filters, num_filters, learn_shortcut=False, **conv_params)]
    ...
    def forward(self, x):
        ...
        """
        # style_f = F.interpolate((self.model1(x)), scale_factor=4, mode='nearest')
        style_f = self.model1(x)
        return style_f
...
class ContentEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(num_downsamples):
            ...
            dims *= 2
        model += [Conv2dBlock(dims, dims, 4, 2, 1, **conv_params)]
...
if __name__ == '__main__':
    model = Generator(gen_cfg={}, data_cfg={})
    data = {}
